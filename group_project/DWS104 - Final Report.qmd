...
- We will conduct a two-side test, with the Null Hypothesis being $𝐻_0$: $𝜇_1$ = $𝜇_2$ that there is no significant difference in the certainty score means between the two samples. The Alternative Hypothesis $𝐻_0$: $𝜇_1$ \neq $𝜇_2$ is that there is a statistically significant difference.
...
This is confirmed by the results of the Welch’s t-test presented in @tbl-welch-test.
```{python}
#| label: tbl-welch-test
#| tbl-cap: "Welch’s two-sample t-test comparing certainty scores between conspiracy and non-conspiracy tweets"
